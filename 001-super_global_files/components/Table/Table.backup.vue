...
export default {
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      console.log(props.tableDetails.table.tbody[1]);
    });
...
<template>
  <div class="tableResponsive">
        ...
        <tr v-for="(bodyRow, indexBodyRow) in tableDetails.table.tbody" :key="indexBodyRow">
          <td>
            <table>
              <thead>
                <tr>
                  <th :colspan="bodyRow.length + tableDetails.table.thead.length">{{ bodyRow[0].name }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(bodyCell, indexBodyCell) in bodyRow" :key="indexBodyCell">
                  <template v-for="(headCell, indexHeadCell) in tableDetails.table.thead" :key="indexHeadCell">
                    <template v-if="headCell[indexBodyCell]">
                      <th
                        :class="[headCell[indexBodyCell].class ? headCell[indexBodyCell].class : '']"
                        :rowspan="
    ...
    <table v-else class="scssecoTable desktop">
      ...
      <tbody>
        <tr v-for="(bodyRow, index) in tableDetails.table.tbody2">
          <td v-for="(bodyCell, index) in bodyRow" :key="index" :colspan="bodyCell.colspan ? bodyCell.colspan : null">
            {{ bodyCell }}
          </td>
        </tr>
      </tbody>
